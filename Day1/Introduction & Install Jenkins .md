...
## Jenkins: Windows Node Setup + Pipeline Job + Script Execution

This document provides a complete, step-by-end flow to:

Create a Windows build node in Jenkins

Create a Pipeline Job

Execute PowerShell commands + file-copy script inside the pipeline

Run the job with and without SCM (Git)

## 1. Create Windows Node (Agent) in Jenkins
Prerequisites

Windows machine reachable from Jenkins master

Java installed (java -version)

Jenkins master can reach agent on port TCP inbound (default: random)

Steps

Log in to Jenkins → Manage Jenkins
...
On node details page → Download agent.jar

On Windows machine, run:

java -jar agent.jar -jnlpUrl http://<JENKINS_URL>/computer/<NODE_NAME>/slave-agent.jnlp -secret <SECRET>


Node will appear online in Jenkins.

## 2. Create a Pipeline Job Without Git (SCM = None)

Go to New Item

Select Pipeline

Name: windows-pipeline-no-scm
...
## 3. Create a Pipeline Job With Git SCM

Go to New Item

Select Pipeline

Name: windows-pipeline-git
...
Jenkins will look for a file named Jenkinsfile at repo root.

## 4. PowerShell Script Used in Pipeline
Local Script Example (movefiles.ps1)
...
## 5. Pipeline Executing PowerShell Commands + File Copy
Jenkinsfile
pipeline {
  agent { label 'windows' }
...
## 6. Freestyle vs Pipeline (Quick Difference)
Feature	Freestyle Job	Pipeline Job
UI-based configuration	Yes	No (code-based)
Complex workflows	Limited	Excellent
Version control of build logic	No	Yes (via Jenkinsfile)
Best for	Simple builds	CI/CD automation
## 7. Key Notes

Use labels to target specific nodes

Windows pipelines must use powershell step

Git pipelines require a Jenkinsfile in the repo

Node must stay online for job execution
